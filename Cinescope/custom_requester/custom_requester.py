...
class CustomRequester:
    ...
    def send_request(self, method, endpoint,json = None, data=None, params = None, expected_status=200, need_logging=True):
            ...
            if hasattr(request, 'body') and request.body is not None:
                if isinstance(request.body, bytes):
                    try:
                        ...
                        body = repr(request.body)  # на случай странных байт
                elif isinstance(request.body, str):
                    body = request.body

                if body:  # если что-то есть — показываем
                    body = f"-d '{body}' \\\n"
                else:
                    body = ""
...
class MoviesRequester(CustomRequester):
    def __init__(self, session,base_url):
